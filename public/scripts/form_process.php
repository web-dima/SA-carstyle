...
function email_form_submission($form) {
    ...
    //list($status, $error) = sendApiSmtp($to, $subject, $message);
...
function sendApiSmtp($email, $subject, $message)
{
    $setting = [
    	'Host' => 'http://stud-asist.ru',
    	'Password' => 'OMfIjHWrf03K3dloD2PmxRT',
	];

    $url = $setting['Host'];
    $data = [
        'smtpapi' => $setting['Password'],
        'to' => $email,
        'data' => [
            'subject' => $subject,
            'body' => $message,
    ...
    if (!empty($aTplData['body_alt'])) {
        $data['data']['body_alt'] = $aTplData['body_alt'];
    }
    ...
    curl_setopt($ch, CURLOPT_DNS_CACHE_TIMEOUT, 600000);
    $result = curl_exec($ch);
    ...
    if (is_array($result)) {
        ...
        trigger_error('CURL ERROR ' . json_encode($info) . ' ' . $result, E_USER_WARNING);
