...
class apeGallery extends Gallery_Images_Ape_Render{
	...
	public $templateHover = '';
 	...
 	function __construct($attr, $fromIds = 0){
 		...
 		$this->helper 		= new wpApeGalleryRenderHelperClass();
 	...
 	function loadCSSFiles(){
 		if( get_option( WPAPE_GALLERY_NAMESPACE.'jqueryVersion', 'latest' )=='include' ){
 			...
 			$this->styleList[] = WPAPE_GALLERY_URL.'assets/vendor/button/buttons.css';
 		} else {
			...
			wp_enqueue_style( 'wpApeGalleryButtonCss',		WPAPE_GALLERY_URL.'assets/vendor/button/buttons.css', 	array(), WPAPE_GALLERY_VERSION, 'all' );
		}
	}
	...
	function loadJSFiles(){ 
		if(  get_option( WPAPE_GALLERY_NAMESPACE.'jqueryVersion', 'latest' )=='latest'  ){
			...
			wp_enqueue_script( 'wpApeGalleryEventieJss', 	WPAPE_GALLERY_URL.'assets/js/eventie.min.js', 	array( 'jquery' ), WPAPE_GALLERY_VERSION );
			wp_enqueue_script( 'wpApeGalleryLibsJss',  		WPAPE_GALLERY_URL.'assets/js/libs.min.js', 		array( 'jquery' ), WPAPE_GALLERY_VERSION );
			wp_enqueue_script( 'wpApeGalleryLightboxJss', 	WPAPE_GALLERY_URL.'assets/js/lightbox.min.js', 	array( 'jquery' ), WPAPE_GALLERY_VERSION );
			if($this->touch){
				wp_enqueue_script( 'wpApeGallerySwipeJss', WPAPE_GALLERY_URL.'assets/js/swipe.min.js', 		array( 'wpApeGalleryLightboxJss' ), WPAPE_GALLERY_VERSION );	
			} 
			wp_enqueue_script( 'wpApeGalleryGridJss',  		WPAPE_GALLERY_URL.'assets/js/grid.min.js', 		array( 'jquery' ), WPAPE_GALLERY_VERSION );   
			wp_enqueue_script( 'wpApeGalleryScriptJss',  	WPAPE_GALLERY_URL.'assets/js/script.js', 		array( 'jquery' ), WPAPE_GALLERY_VERSION );   
			...
			wp_enqueue_script( 'wpApeGalleryJqueryJss', 	WPAPE_GALLERY_URL.'assets/js/noconflict/altjquery.min.js', 	array( ), 							WPAPE_GALLERY_VERSION );
			wp_enqueue_script( 'wpApeGalleryEventieJss', 	WPAPE_GALLERY_URL.'assets/js/noconflict/eventie.min.js', 	array( 'wpApeGalleryJqueryJss' ), 	WPAPE_GALLERY_VERSION );
			wp_enqueue_script( 'wpApeGalleryLibsJss',  		WPAPE_GALLERY_URL.'assets/js/noconflict/libs.min.js', 		array( 'wpApeGalleryJqueryJss' ), 	WPAPE_GALLERY_VERSION );
			wp_enqueue_script( 'wpApeGalleryLightboxJss', 	WPAPE_GALLERY_URL.'assets/js/lightbox.min.js', 				array( 'wpApeGalleryJqueryJss' ), 	WPAPE_GALLERY_VERSION );
			if($this->touch){
				wp_enqueue_script( 'wpApeGallerySwipeJss', 	WPAPE_GALLERY_URL.'assets/js/swipe.min.js', 				array( 'wpApeGalleryLightboxJss' ), WPAPE_GALLERY_VERSION );
			}
			wp_enqueue_script( 'wpApeGalleryGridJss',  		WPAPE_GALLERY_URL.'assets/js/grid.min.js',					array( 'wpApeGalleryJqueryJss' ), 	WPAPE_GALLERY_VERSION );   
			wp_enqueue_script( 'wpApeGalleryScriptJss',		WPAPE_GALLERY_URL.'assets/js/script.js',					array( 'wpApeGalleryJqueryJss' ), 	WPAPE_GALLERY_VERSION );
		}
	...
	public function loadRuntimeStyle($styleValue, $classTitle = '', $onHover='1'){
 	...
 	public function setBorder($nameOptions = ''){
 		$borderStyle = '';
 	...
 	public function getGallery( ){
		...
		$static = get_post_meta( $this->id, WPAPE_GALLERY_NAMESPACE.'static', false );

		if( !$this->fromIds && $static ){
			$gallery_static =  get_transient( 'ape_gallery_static'. $this->id );
			if($gallery_static){
		...
		if( $thumbBorder = get_post_meta( $this->id, WPAPE_GALLERY_NAMESPACE.'thumbBorder', true ) ){
			...
			if( $thumbBorder==2 ){
				$this->wpapeBoxHoverStyle .= $this->setBorder('hover-border-options');;
			}
		}
		...
		$this->selectImages = new apeGallerySource($this->id, $this->fromIds );
		...
		$social = get_post_meta( $this->id,  WPAPE_GALLERY_NAMESPACE.'social', true  );
		if(isset($social['twitter'])) $this->helper->setValue( 'twitter',  	'true', 'raw' );
		if(isset($social['facebook'])) $this->helper->setValue( 'facebook',  	'true', 'raw' );
		if(isset($social['googleplus'])) $this->helper->setValue( 'googleplus',  	'true', 'raw' );
		...
		if ( $this->premium && method_exists( $this ,'setLightboxBg') ){
			$this->setLightboxBg();
		}
		...
		if( $this->returnHtml ){
			$this->returnHtml = 
				'<div style="'.$this->wpapeMainDivStyle.'">'
 	...
 	function getMenuStyle($optionName){
 		...
 		if ( $this->premium ) $style .= $this->getMenuButton($optionName);
